...
class ListIterator
	: public ValueIterator<std::list<std::string>>
...
public:
	...
	// source variable.
	static bool GetMemberInvoker(MQVarPtr VarPtr, const char* Member, char* Index, MQTypeVar& Dest);

protected:
	// Return an iterator on the list for a particular key. Return false if the key is not found.
...
class List
	: public Collection<std::list<std::string>, std::string, std::string, ValueIterator<std::list<std::string>>>
...
public:
	...
	bool Insert(const size_t position, const Container& sequence)
	{
		...
		m_container.insert(it, sequence.cbegin(), sequence.cend());

		return true;
	}

	// Sort the list into ascending order. The list is altered in place.
	void Sort();

	// The elements in the list are reversed. The list is reversed in place.
	void Reverse();
	...
	// Used by the testing framework to call GetMember through the
	// source variable.
	static bool GetMemberInvoker(MQVarPtr VarPtr, const char* Member, const char* Index, MQTypeVar& Dest);

protected:
	// Return an iterator on the list.
	virtual std::unique_ptr<ValueIterator<std::list<std::string>>> GetNewIterator(
		const std::list<std::string>& refCollection) const override;
...
private:
	...
	bool IndexValueFromString(const std::string& stringIndex, size_t* longIndex) const;
