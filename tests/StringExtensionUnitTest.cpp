...
public:
	...
	TEST_METHOD(SplitOnSpaceIntoOneString)
	{
		...
		Assert::AreEqual(static_cast<size_t>(2), container->size(), L"Expected Split() to return 2 elements.");
	}

	//
	// Test a complex string delimited by a space.
	//
	...
	TEST_METHOD(SplitOnSpaceIntoTenStrings)
	{
		...
		Assert::AreEqual(static_cast<size_t>(10),
			container->size(),
	...
	TEST_METHOD(SplitOnCommaAndSpaceRemoveEmpty)
	{
		...
		Assert::AreEqual(static_cast<size_t>(10), container->size(), L"Expected Split() to return 10 elements.");
	}
	...
	TEST_METHOD(SplitOnCommaWithOnlyDelimiters)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), container->size(), L"Expected Split() to return 0 elements.");
	}
	...
	TEST_METHOD(SplitOnEmptyString)
	{
		...
		Assert::AreEqual(static_cast<size_t>(1), container->size(), L"Expected Split() to return 1 elements.");
	}
	...
	TEST_METHOD(SplitOnSpaceWithQuotedCharacter)
	{
		...
		Assert::AreEqual(static_cast<ptrdiff_t>(1), std::count(container->at(0).cbegin(), container->at(0).cend(), '\\'),
	...
	TEST_METHOD(SplitOnSpaceWithSpacesInAQuotedString)
	{
		...
		Assert::AreEqual(static_cast<size_t>(1), container->size(), L"Expected Split() to return 1 element.");
	}
	...
	TEST_METHOD(SplitOnSpaceWithMixedQuotedAndUnquotedStrings)
	{
		...
		Assert::AreEqual(static_cast<size_t>(3), container->size(), L"Expected Split() to return 3 elements.");
	}
...
public:
	...
	TEST_METHOD(TrimStartNoTrim)
	{
		...
		Assert::AreEqual(extension.Contents(), newExtension->Contents(), L"Expected trimmed instance to be the same!");
	}
	...
	TEST_METHOD(TrimStartBlanks)
	{
		...
		Assert::AreNotEqual(extension.Contents(), newExtension->Contents(), L"Expected trimmed instance to not be the same!");
	}
	...
	TEST_METHOD(TrimStartAlpha)
	{
		...
		Assert::AreEqual(std::string("1234567890"), newExtension->Contents(), L"Expected trimmed instance to be the same!");
	}
	...
	//
	// Test trimming blanks from both ends of a string in which there are no
	...
	TEST_METHOD(TrimBlanks)
	{
		...
		Assert::AreEqual(std::string("Hello, world!"), newExtension->Contents(), L"Expected trimmed instance to be the same!");
	}
