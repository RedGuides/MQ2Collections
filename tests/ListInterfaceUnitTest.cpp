...
{
	mq::Test_InitializeDataAPI();
}

TEST_MODULE_CLEANUP(ModuleCleanup)
{
	mq::Test_ShutdownDataAPI();
}
...
public:
	...
	TEST_METHOD(Constructor)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), l.Count(), L"There must be zero elements in the list.");
	}
	...
	TEST_METHOD(EmptySpliceWithZeroIndex)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), splice->Count(), L"Splice must be empty.");
	}
	...
	TEST_METHOD(SpliceFromEnd)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), splice->Count(), L"Splice must have zero elements.");
	}
	...
	TEST_METHOD(SpliceFromMiddle)
	{
		...
		auto splice = l.Splice(l.Count() / 2);

		// See if the returned splice exists.
		Assert::IsNotNull(splice.get(), L"Splice must not be a nullptr.");

		size_t splice_length = static_cast<size_t>(std::ceil(l.Count() / 2.0));
	...
	TEST_METHOD(SpliceFromStartWithZeroLength)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), splice->Count(), L"Returned splice must have a length of zero.");
	}
	...
	TEST_METHOD(SpliceFromMiddleWithLengthOfOne)
	{
		...
		// See if the returned splice exists.
		Assert::IsNotNull(splice.get(), L"Splice must not be a nullptr.");
		Assert::AreEqual(static_cast<size_t>(1), splice->Count(), L"Splice length is incorrect.");
	...
	TEST_METHOD(IndexOnEmptyList)
	{
		...
		// Look up 'Zero'  -- it should return -1 indicating the element is not in the list.
		Assert::AreEqual(-1, l.Index("Zero"), L"Index of 'Zero' should be -1.");
	}
	...
	TEST_METHOD(IndexForKnownElements)
	{
		...
		{
			Assert::AreEqual(static_cast<int>(index), l.Index(elements[index]), L"Index was incorrect for element.");
		}
	}
	...
	TEST_METHOD(IndexForUnknownElement)
	{
		...
		// Look up 'Zero'  -- it should return -1 indicating the element is not in the list.
		Assert::AreEqual(-1, l.Index(std::string("Zero")), L"Index of 'Zero' should be -1.");
	}
	...
	TEST_METHOD(ItemCompareEachEntry)
	{
		...
		CompareListToElements(l, elements);
	}
	...
	TEST_METHOD(InsertEmptyListIntoEmptyList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), l.Count(), L"Final list should be empty.");
	}
	...
	TEST_METHOD(InsertEmptyListAtStartOfList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(5), l.Count(), L"Target list should be 5 entries long.");
	...
	TEST_METHOD(InsertListAtStartOfAnotherList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(8), l.Count(), L"Target list should be 8 entries long.");

		// Verify the elements in the list.
		CompareListToElements(l, insertedelements);
	}
	...
	TEST_METHOD(InsertIntoEmptyList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(3), l.Count(), L"Target list should be 3 entries long.");
	...
	TEST_METHOD(SortEmptyList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), l.Count(), L"List must have zero entries.");
	}
	...
	{
		std::vector<std::string> sortedelements =
		{
			"Five",
			"Four",
			"One",
		...
		// List must be 5 in length.
		Assert::AreEqual(static_cast<size_t>(5), l.Count(), L"Sorted list must be same length.");

		// Verify the elements are sorted.
		CompareListToElements(l, sortedelements);
	}
	...
	TEST_METHOD(ReverseAnEmptyList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), l.Count(), L"Reversed list must be empty.");
	}
	...
	{
		std::vector<std::string> reservsedelements =
		{
			"Five",
			"Four",
			"Three",
		...
		// List must be 5 in length.
		Assert::AreEqual(static_cast<size_t>(5), l.Count(), L"Reversed list must be same length.");

		// Verify the elements are reversed.
		CompareListToElements(l, reservsedelements);
	}
	...
	{
		std::vector<std::string> appendedelements =
		{
			"One"
		};
		...
		Assert::AreEqual(static_cast<size_t>(1), l.Count(), L"List length must be one.");

		// Verify the elements are correct.
		CompareListToElements(l, appendedelements);
	}

	//
	// Append a quoted item to an empty list.
	//
	// Result: new elements should be added to the list.
	//
	...
	TEST_METHOD(AppendToList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(7), l.Count(), L"List length must be seven.");
	...
	TEST_METHOD(RemoveFromEmptyList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), l.Remove("Zero"), L"Can not remove item from empty list.");

		// Length of the list should be zero.
		Assert::AreEqual(static_cast<size_t>(0), l.Count(), L"List must be empty.");
	}
	...
	TEST_METHOD(RemoveItemNotInList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), l.Remove(std::string("Zero")), L"No elements should be removed from the list");

		// Verify no elements were removed.
		CompareListToElements(l, removedelements);
	}
	...
	TEST_METHOD(RemoveFirstItem)
	{
		...
		Assert::AreEqual(static_cast<size_t>(1), l.Remove(std::string("One")), L"Only one element should be removed from the list");
	...
	TEST_METHOD(RemoveLastItem)
	{
		...
		Assert::AreEqual(static_cast<size_t>(1), l.Remove(std::string("Five")), L"Only one element should be removed from the list");
	...
	TEST_METHOD(RemoveMiddleItem)
	{
		...
		Assert::AreEqual(static_cast<size_t>(1), l.Remove(std::string("Three")), L"Only one element should be removed from the list");
	...
	TEST_METHOD(RemoveMultiple)
	{
		...
		Assert::AreEqual(static_cast<size_t>(2), l.Remove(std::string("Three")), L"Two elements should be removed from the list.");
	...
	TEST_METHOD(ReplaceInEmptyList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), l.Replace("Zero", "Empty"), L"Empty list replace should fail.");
	}
	...
	{
		std::vector<std::string> replacedelements =
		{
			"Zero",
			"Two",
			"Three",
		...
		Assert::AreEqual(static_cast<size_t>(1), l.Replace("One", "Zero"), L"Only one element should be replaced in the list.");

		// Verify the element was replaced.
		CompareListToElements(l, replacedelements);
	}
	...
	TEST_METHOD(ReplaceLastElement)
	{
		...
		Assert::AreEqual(static_cast<size_t>(1), l.Replace("Five", "Last"), L"Only one element should be replaced in the list.");
	...
	TEST_METHOD(ReplaceMiddleElement)
	{
		...
		Assert::AreEqual(static_cast<size_t>(1), l.Replace("Three", "Middle"), L"Only one element should be replaced in the list.");
	...
	TEST_METHOD(ReplaceItemNotInList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), l.Replace("Zero", "Empty"), L"No elements should be replaced in the list.");
	...
	TEST_METHOD(ReplaceMultiple)
	{
		...
		Assert::AreEqual(static_cast<size_t>(2), l.Replace("Three", "Zero"), L"Two elements should be replaced in the list.");
	...
	TEST_METHOD(PopAllFromTail)
	{
		...
		for (const std::string& item : elements)
		{
			...
			Assert::AreEqual(item, *pitem, L"Item returned is incorrect.");
		}
	...
	TEST_METHOD(CountOfOnEmptyList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), l.CountOf("Zero"), L"CountOf should return zero.");
	}
	...
	TEST_METHOD(CountOfKnownElements)
	{
		...
		// Verify that each element occurs once in the list.
		for (auto it : elements)
		{
			Assert::AreEqual(static_cast<size_t>(1), l.CountOf(it), L"CountOf should return one.");
		}
	}
...
private:
	...
	void AppendFive(List& l) const
	{
		...
		Assert::AreEqual(static_cast<size_t>(1), l.Count(), L"There must be one elements in the list");

		l.Append(std::string("Two"));
		Assert::AreEqual(static_cast<size_t>(2), l.Count(), L"There must be two elements in the list");

		l.Append(std::string("Three"));
		Assert::AreEqual(static_cast<size_t>(3), l.Count(), L"There must be three elements in the list");

		l.Append(std::string("Four"));
		Assert::AreEqual(static_cast<size_t>(4), l.Count(), L"There must be four elements in the list");

		l.Append(std::string("Five"));
		Assert::AreEqual(static_cast<size_t>(5), l.Count(), L"There must be five elements in the list");
	}
	...
	void CompareListToElements(const List& l, const std::vector<std::string>& elements) const
	{
