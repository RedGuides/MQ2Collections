...
public:
	...
	TEST_METHOD(CountOnEmptyList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Count", nullptr, dest);
		Assert::IsTrue(bResult, L"Count invocation failed.");
		Assert::AreEqual(0, dest.Int, L"Count should be zero.");
	}

	//
	// Call the Count method on a populated list.
	//
	...
	TEST_METHOD(ClearEmptyList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Clear", nullptr, dest);
	...
	TEST_METHOD(ContainsAnElement)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Contains", "C", dest);
	...
	TEST_METHOD(EmptyListDoesNotContainAnElement)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Contains", "A", dest);
		Assert::IsTrue(bResult, L"Contains invocation failed.");
		Assert::AreEqual(0, dest.Int, L"Contains should return false.");
	}

	//
	// Create a list, populate it and verify it does not contain a known element.
	//
	// Result: the list should not contain the element.
	//
	...
	TEST_METHOD(DoesNotContainsAnElement)
	{
		...
		// Test Contains on an item we know can not be in the list.
		bool bResult = List::GetMemberInvoker(source, "Contains", "F", dest);
		Assert::IsTrue(bResult, L"Contains invocation failed.");
		Assert::AreEqual(0, dest.Int, L"Contains should return false.");
	}
	...
	TEST_METHOD(EmptySpliceWithZeroIndex)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "0", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");

		std::unique_ptr<List> splice(static_cast<List*>(dest.Ptr));

		// Length of returned list must be zero.
		Assert::AreEqual(static_cast<size_t>(0), splice->Count(), L"Length of returned splice must be zero.");
	}
	...
	TEST_METHOD(SpliceFromEnd)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "100", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	TEST_METHOD(SpliceFromMiddle)
	{
		...
		// Retrieve a splice starting past the origin of the list.
		bool bResult = List::GetMemberInvoker(source, "Splice", "2", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");

		std::unique_ptr<List> splice(static_cast<List*>(dest.Ptr));

		size_t splice_length = static_cast<size_t>(std::ceil(pl->Count() / 2.0));
	...
	TEST_METHOD(EmptySpliceWithZeroLength)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "0,0", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	TEST_METHOD(EmptySpliceWithPositiveLength)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "0,1", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	TEST_METHOD(EmptySpliceWithPositiveOffsetAndZeroLength)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "10,0", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	TEST_METHOD(EmptySpliceWithPositiveOffsetAndPositiveLength)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "10,10", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	TEST_METHOD(SpliceFromStartPastTheEnd)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "0, 100", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	TEST_METHOD(SpliceFromStartForLengthOfList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "0, 5", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	TEST_METHOD(SpliceFromStartWithZeroLength)
	{
		...
		Assert::AreEqual(static_cast<size_t>(0), splice->Count(), L"Splice must have a length of zero.");
	}
	...
	TEST_METHOD(SpliceFromStartForLengthOfOne)
	{
		...
		Assert::AreEqual(static_cast<size_t>(1), splice->Count(), L"Splice must have a length of one.");
	...
	TEST_METHOD(SpliceFromEndForLengthOfOne)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "4, 1", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	TEST_METHOD(SpliceFromEndForLengthOfZero)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "4, 0", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	TEST_METHOD(SpliceFromEndForLargeLength)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "4, 100", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	//
	// Result: the splice should be of length one.
	//
	...
	TEST_METHOD(SpliceFromMiddleWithLengthOfOne)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "2, 1", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	//
	// Result: the splice should be of length zero.
	//
	...
	TEST_METHOD(SpliceFromMiddleWithLengthOfZero)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Splice", "2, 0", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	TEST_METHOD(SpliceFromMiddleForRemainder)
	{
		...
		// Retrieve a splice from the middle for a length of one.
		bool bResult = List::GetMemberInvoker(source, "Splice", "2, 3", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
		...
		Assert::AreEqual(static_cast<size_t>(3), splice->Count(), L"Splice must have a length of three.");
	...
	TEST_METHOD(SpliceFromMiddleForLargeLength)
	{
		...
		// Retrieve a splice from the middle for a length of one.
		bool bResult = List::GetMemberInvoker(source, "Splice", "2, 100", dest);
		Assert::IsTrue(bResult, L"Splice invocation failed.");
		Assert::IsNotNull(dest.Ptr, L"Splice should not return a nullptr.");
	...
	TEST_METHOD(IndexOnEmptyList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Index", "Zero", dest);
	...
	TEST_METHOD(IndexQuotedItemOnEmptyList)
	{
		...
		// Index of 'Zero' should be -1, indicating it was not found.
		bool bResult = List::GetMemberInvoker(source, "Index", "\"Zero, Item\"", dest);
		Assert::IsTrue(bResult, L"Index invocation failed.");
	...
	TEST_METHOD(IndexForKnownElements)
	{
		...
		{
			bool bResult = List::GetMemberInvoker(source, "Index", elements[index].c_str(), dest);
			Assert::IsTrue(bResult, L"Index invocation failed.");
			Assert::AreEqual(index, dest.Int, L"Index is incorrect for element.");
		}
	}
	...
	TEST_METHOD(ItemEmptyList)
	{
		...
		// Index of 'Zero' should be -1, indicating it was not found.
		bool bResult = List::GetMemberInvoker(source, "Item", "0", dest);
	...
	TEST_METHOD(ItemCompareEachEntry)
	{
		...
		CompareListToElements(source, elements);
	}
	...
	TEST_METHOD(ItemAfterListEnd)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Item", "100", dest);
		Assert::IsTrue(bResult, L"Item invocation failed.");
	...
	TEST_METHOD(InsertEmptyListIntoEmptyList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Insert", "0,", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");

		Assert::AreEqual(static_cast<size_t>(0), pl->Count(), L"List must have zero elements.");
	}
	...
	TEST_METHOD(InsertEmptyListAtStartOfList)
	{
		...
		Assert::AreEqual(static_cast<size_t>(5), pl->Count(), L"List must have five elements.");
	...
	TEST_METHOD(InsertEmptyListAtEndOfList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Insert", "5,", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");
	...
	TEST_METHOD(InsertEmptyListInMiddleOfList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Insert", "3,", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");
	...
	TEST_METHOD(InsertListAtStartOfAnotherList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Insert", "0,One,Two,Three", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");

		Assert::AreEqual(static_cast<size_t>(8), pl->Count(), L"List must have eight elements.");
	...
	TEST_METHOD(InsertListAtStartOfAnotherListWithCustomDelimiter)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Delimiter", "|", old_delim);
	...
	TEST_METHOD(InsertListWithQuotedItemsAtStartOfAnotherList)
	{
		...
		// Insert a sequence at the start of the list.
		bool bResult = List::GetMemberInvoker(source, "Insert", "0,One,\"Two, with a comma\",\"Three with a \\\" quote!\"", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");
	...
	TEST_METHOD(InsertListAtEndOfAnotherList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Insert", "5,One,Two,Three", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");
	...
	TEST_METHOD(InsertListWithQuotedItemsAtEndOfAnotherList)
	{
		...
		// Insert a sequence at the end of the list.
		bool bResult = List::GetMemberInvoker(source, "Insert", "5,One,\"Two, with a comma\",\"Three with a \\\" quote!\"", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");
	...
	TEST_METHOD(InsertListInMiddleOfAnotherList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Insert", "3,One,Two,Three", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");
	...
	TEST_METHOD(InsertListWithQuotedItemsInMiddleOfAnotherList)
	{
		...
		// Insert a sequence in the middle of the list.
		bool bResult = List::GetMemberInvoker(source, "Insert", "3,One,\"Two, with a comma\",\"Three with a \\\" quote!\"", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");
	...
	TEST_METHOD(InsertPastEnd)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Insert", "6,One,Two,Three", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
	...
	TEST_METHOD(InsertWithQuotedItemsPastEnd)
	{
		...
		// Insert a sequence past the end of the list.
		bool bResult = List::GetMemberInvoker(source, "Insert", "6,One,\"Two, has a comma\",\"Three has a quote \\\"\"", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(0, dest.Int, L"Insert should return false.");
	...
	TEST_METHOD(InsertIntoEmptyList)
	{
		...
		// Insert a sequence past the end of the list.
		bool bResult = List::GetMemberInvoker(source, "Insert", "0,A,B,C,D,E", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");
	...
	TEST_METHOD(InsertWithQuotedItemsIntoEmptyList)
	{
		...
		// Insert a sequence past the end of the list.
		bool bResult = List::GetMemberInvoker(source, "Insert", "0,\"A, 1, 2, 3, 4\",\"B, \\\'\\\" quote! @\",\"B, '\\\" quote! @\",C,D,E", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");

		Assert::AreEqual(static_cast<size_t>(6), pl->Count(), L"List must have six elements.");
	...
	TEST_METHOD(SortEmptyList)
	{
		...
		// Sort the empty list.
		bool bResult = List::GetMemberInvoker(source, "Sort", nullptr, dest);
	...
	TEST_METHOD(SortList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Insert", "0,F,G,H", dest);
		Assert::IsTrue(bResult, L"Insert invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Insert should return true.");
	...
	TEST_METHOD(ReverseAnEmptyList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Reverse", nullptr, dest);
	...
	TEST_METHOD(AppendToEmptyList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Append", "A,B,C,D,E", dest);
	...
	TEST_METHOD(AppendQuotedItemsToEmptyList)
	{
		...
		// Append a sequence onto an empty list.
		bool bResult = List::GetMemberInvoker(source, "Append", "\"A\",\"B \\\"\",\"C with a single \\\' quote\",\"C with a single ' quote\",\"D, has some commas, right\",\"E has quotes \\\"and\\\" commas 1,2,3\"", dest);
		Assert::IsTrue(bResult, L"Append invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Append should return true.");
	...
	TEST_METHOD(AppendEmbeddedQuotedItemToEmptyList)
	{
		...
		// Append a sequence onto an empty list.
		bool bResult = List::GetMemberInvoker(source, "Append", "\"Macro's C'omm'a Issue's\"", dest);
		Assert::IsTrue(bResult, L"Append invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Append should return true.");

		Assert::AreEqual(static_cast<size_t>(1), pl->Count(), L"List must have one element.");
	...
	TEST_METHOD(AppendToList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Append", "One,Two,Three", dest);
		Assert::IsTrue(bResult, L"Append invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Append should return true.");
	...
	TEST_METHOD(AppendQuotedItemsToList)
	{
		...
		// Append a sequence onto the list.
		bool bResult = List::GetMemberInvoker(source, "Append", "\"One\",\"Two, with a comma\",\"Three with a \\\" quote!\"", dest);
		Assert::IsTrue(bResult, L"Append invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Append should return true.");
	...
	TEST_METHOD(AppendEmptySequenceToEmptyList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Append", "", dest);
		Assert::IsTrue(bResult, L"Append invocation failed.");
	...
	TEST_METHOD(RemoveFromEmptyList)
	{
		...
		// Remove an item from an empty list.
		bool bResult = List::GetMemberInvoker(source, "Remove", "A", dest);
	...
	TEST_METHOD(RemoveItemNotInList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Remove", "Zero", dest);
		Assert::IsTrue(bResult, L"Remove invocation failed.");
	...
	TEST_METHOD(RemoveFirstItem)
	{
		...
		Assert::AreEqual(static_cast<size_t>(4), pl->Count(), L"List must have four elements.");
	...
	TEST_METHOD(RemoveLastItem)
	{
		...
		// Remove the last element from the list.
		bool bResult = List::GetMemberInvoker(source, "Remove", "E", dest);
		Assert::IsTrue(bResult, L"Remove invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Remove should return one element removed.");
	...
	TEST_METHOD(RemoveMiddleItem)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Remove", "C", dest);
		Assert::IsTrue(bResult, L"Remove invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Remove should return one element removed.");
	...
	TEST_METHOD(RemoveMultiple)
	{
		...
		std::vector<std::string> appenditems =
		{
			"A",
			"B",
			"C",
		...
		for (const std::string& item : appenditems)
		{
			bool bResult = List::GetMemberInvoker(source, "Append", item.c_str(), dest);
			Assert::IsTrue(bResult, L"Append invocation failed.");
			Assert::AreEqual(1, dest.Int, L"Append should return true.");
		}
	...
	TEST_METHOD(EraseFromEmptyList)
	{
		...
		// Remove an item from an empty list.
		bool bResult = List::GetMemberInvoker(source, "Erase", "0", dest);
	...
	TEST_METHOD(EraseMiddleItem)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Erase", "2", dest);
		Assert::IsTrue(bResult, L"Erase invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Erase should return true.");
	...
	TEST_METHOD(EraseLastItem)
	{
		...
		// Erase the middle element from the list.
		bool bResult = List::GetMemberInvoker(source, "Erase", "4", dest);
		Assert::IsTrue(bResult, L"Erase invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Erase should return true.");
	...
	TEST_METHOD(EraseItemOutOfBounds)
	{
		...
		// Erase the middle element from the list.
		bool bResult = List::GetMemberInvoker(source, "Erase", "5", dest);
		Assert::IsTrue(bResult, L"Erase invocation failed.");
		Assert::AreEqual(0, dest.Int, L"Erase should return false.");
	...
	TEST_METHOD(ReplaceInEmptyList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Replace", "A,Empty", dest);
	...
	TEST_METHOD(ReplaceFirstElement)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Replace", "A,First", dest);
		Assert::IsTrue(bResult, L"Replace invocation failed.");
	...
	TEST_METHOD(ReplaceLastElement)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Replace", "E,Last", dest);
		Assert::IsTrue(bResult, L"Replace invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Replace should return one entry replaced.");
	...
	TEST_METHOD(ReplaceMiddleElement)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Replace", "C,Middle", dest);
		Assert::IsTrue(bResult, L"Replace invocation failed.");
		Assert::AreEqual(1, dest.Int, L"Replace should return one entry replaced.");
	...
	TEST_METHOD(ReplaceItemNotInList)
	{
		...
		// Replace an item that is not in the list.
		bool bResult = List::GetMemberInvoker(source, "Replace", "Zero,Empty", dest);
		Assert::IsTrue(bResult, L"Replace invocation failed.");
	...
	TEST_METHOD(ReplaceMultiple)
	{
		...
		MQVarPtr source;
		MQTypeVar dest = {};
		std::vector<std::string> appendelements =
		{
			"A",
			"B",
			"C",
		...
		for (const std::string& item : appendelements)
		{
			bool bResult = List::GetMemberInvoker(source, "Append", item.c_str(), dest);
			Assert::IsTrue(bResult, L"Append invocation failed.");
			Assert::AreEqual(1, dest.Int, L"Append should return true.");
		...
		Assert::AreEqual(static_cast<size_t>(10), pl->Count(), L"List must have ten elements.");
	...
	TEST_METHOD(HeadOfEmptyList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Head", nullptr, dest);
	...
	TEST_METHOD(HeadOfList)
	{
		...
		Assert::AreEqual(head.c_str(), static_cast<const char*>(dest.Ptr), false, L"Head is not equal to the expected value.");
	}
	...
	TEST_METHOD(PopAllFromHead)
	{
		...
		for (size_t nItem = 0; nItem < elements.size(); ++nItem)
		{
			// Pop the head and verify it is expected value.
			bool bResult = List::GetMemberInvoker(source, "Head", nullptr, dest);
			Assert::IsTrue(bResult, L"Head invocation failed.");

			Assert::AreEqual(elements[nItem].c_str(), static_cast<const char*>(dest.Ptr), false, L"Head is not equal to the expected value.");
		}

		// The list should be empty once all the elements are removed.
		Assert::AreEqual(static_cast<size_t>(0), pl->Count(), L"List must be empty.");
	}
	//
	...
	TEST_METHOD(TailOfEmptyList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "Tail", nullptr, dest);
	...
	TEST_METHOD(TailOfList)
	{
		...
		Assert::AreEqual(tail.c_str(), static_cast<const char*>(dest.Ptr), false, L"Tail is not equal to the expected value.");
	}
	...
	TEST_METHOD(PopAllFromTail)
	{
		...
		for (size_t nItem = 0; nItem < elements.size(); ++nItem)
		{
			...
			Assert::AreEqual(elements[nItem].c_str(), static_cast<const char*>(dest.Ptr), false, L"Tail is not equal to the expected value.");
		}
	...
	TEST_METHOD(CountOfOnEmptyList)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "CountOf", "A", dest);
	...
	TEST_METHOD(CountOfNonexistantElement)
	{
		...
		bool bResult = List::GetMemberInvoker(source, "CountOf", "F", dest);
		Assert::IsTrue(bResult, L"CountOf invocation failed.");
		Assert::AreEqual(0, dest.Int, L"CountOf should return zero.");
	}
	...
	TEST_METHOD(CountOfKnownElements)
	{
		...
		{
			bool bResult = List::GetMemberInvoker(source, "CountOf", item.c_str(), dest);
			Assert::IsTrue(bResult, L"CountOf invocation failed.");
...
private:
	...
	void CompareListAndSplice(const List& l, size_t list_origin, const std::unique_ptr<List>& splice, size_t splice_origin)
	{
	...
	void CompareListToElements(MQVarPtr& source, const std::vector<std::string>& elements) const
	{
		MQTypeVar dest = {};
		...
		for (size_t index = 0; index < elements.size(); ++index)
		{
			...
			bool bResult = List::GetMemberInvoker(source, "Item", item_s.c_str(), dest);
			Assert::IsTrue(bResult, L"Item invocation failed.");
			Assert::IsNotNull(dest.Ptr, L"Returned item is a null pointer.");

			int compare_result = strcmp(static_cast<const char*>(dest.Ptr), elements[index].c_str());
